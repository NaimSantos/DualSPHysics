...
tdouble3 JSphCpu::Interaction_PosNoPeriodic(tdouble3 posp1)const{
	if(PeriX){
		if(posp1.x<MapRealPosMin.x)
			posp1=posp1-PeriXinc;
		if(posp1.x>MapRealPosMax.x)
			posp1=posp1+PeriXinc;
	}
	if(PeriY){
		if(posp1.y<MapRealPosMin.y)
			posp1=posp1-PeriYinc;
		if(posp1.y>MapRealPosMax.y)
			posp1=posp1+PeriYinc;
	}
	if(PeriZ){
		if(posp1.z<MapRealPosMin.z)
			posp1=posp1-PeriZinc;
		if(posp1.z>MapRealPosMax.z)
			posp1=posp1+PeriZinc;
	}
	return(posp1);
}
...
//==============================================================================
template<bool sim2d,TpKernel tker>
void JSphCpu::InteractionInOutExtrap_Double(unsigned inoutcount,const int *inoutpart,const byte *cfgzone,
	const tplane3f *planes,const float* width,const tfloat3 *dirdata,float determlimit,StDivDataCpu dvd,
	const unsigned *dcell,const tdouble3 *pos,const typecode *code,const unsigned *idp,tfloat4 *velrhop)
{
...
void JSphCpu::Interaction_InOutExtrapT(byte doublemode,unsigned inoutcount,const int *inoutpart,
	const byte *cfgzone,const tplane3f *planes,const float* width,const tfloat3 *dirdata,float determlimit,
	const unsigned *dcell,const tdouble3 *pos,const typecode *code,const unsigned *idp,tfloat4 *velrhop)
{
	...
	if(doublemode==2){
		if(Simulate2D)
			InteractionInOutExtrap_Single<true ,tker> (inoutcount,inoutpart,cfgzone,planes,width,dirdata,determlimit,dvd,dcell,pos,code,idp,velrhop);
		else
			InteractionInOutExtrap_Single<false,tker> (inoutcount,inoutpart,cfgzone,planes,width,dirdata,determlimit,dvd,dcell,pos,code,idp,velrhop);
	}
	else if(doublemode==3){
		if(Simulate2D)
			InteractionInOutExtrap_Double<true ,tker> (inoutcount,inoutpart,cfgzone,planes,width,dirdata,determlimit,dvd,dcell,pos,code,idp,velrhop);
		else
			InteractionInOutExtrap_Double<false,tker> (inoutcount,inoutpart,cfgzone,planes,width,dirdata,determlimit,dvd,dcell,pos,code,idp,velrhop);
	}
...
/// Realiza interaccion entre ghost inlet/outlet nodes y particulas de fluido. GhostNodes-Fluid
//==============================================================================
void JSphCpu::Interaction_InOutExtrap(byte doublemode,unsigned inoutcount,const int *inoutpart,const byte *cfgzone,const tplane3f *planes,
	const float* width,const tfloat3 *dirdata,float determlimit,const unsigned *dcell,const tdouble3 *pos,const typecode *code,const unsigned *idp,tfloat4 *velrhop)
{
	switch(TKernel){
		case KERNEL_Cubic:
			Interaction_InOutExtrapT<KERNEL_Cubic>(doublemode,inoutcount,inoutpart,cfgzone,planes,width,dirdata,determlimit,dcell,pos,code,idp,velrhop);
			break;
		case KERNEL_Wendland:
			Interaction_InOutExtrapT<KERNEL_Wendland>(doublemode,inoutcount,inoutpart,cfgzone,planes,width,dirdata,determlimit,dcell,pos,code,idp,velrhop);
			break;
		default:
...
//==============================================================================
float JSphCpu::Interaction_InOutZsurf(unsigned nptz,const tfloat3 *ptzpos,float maxdist,float zbottom,const StDivDataCpu &divdata,const tdouble3 *pos,const typecode *code)
{
...
//==============================================================================
template<bool sim2d,TpKernel tker>
void JSphCpu::InteractionBoundCorr_Double(unsigned npb,typecode boundcode,tplane3f plane,tfloat3 direction,float determlimit,
	const StDivDataCpu &dvd,const tdouble3 *pos,const typecode *code,const unsigned *idp,tfloat4 *velrhop)
{
	const int n=int(npb);
	#ifdef _WITHOMP
		#pragma omp parallel for schedule (guided)
	...
	for(int p1=0;p1<n;p1++)
		if(CODE_GetTypeAndValue(code[p1])==boundcode){
			float rhopfinal=FLT_MAX;
			//-Calculates ghost node position.
			tdouble3 pos_p1=pos[p1];
			if(CODE_IsPeriodic(code[p1]))
				pos_p1=Interaction_PosNoPeriodic(pos_p1);
...
//==============================================================================
template<bool sim2d,TpKernel tker>
void JSphCpu::InteractionBoundCorr_Single(unsigned npb,typecode boundcode,tplane3f plane,tfloat3 direction,float determlimit,
	const StDivDataCpu &dvd,const tdouble3 *pos,const typecode *code,const unsigned *idp,tfloat4 *velrhop)
{
	const int n=int(npb);
	#ifdef _WITHOMP
...
//==============================================================================
template<TpKernel tker>
void JSphCpu::Interaction_BoundCorrT(byte doublemode,typecode boundcode,tplane3f plane,tfloat3 direction,float determlimit,const tdouble3 *pos,const typecode *code,const unsigned *idp,tfloat4 *velrhop)
{
	const StDivDataCpu &dvd=DivData;
	//-Interaction GhostBoundaryNodes-Fluid.
	if(doublemode==2){
		if(Simulate2D)
			InteractionBoundCorr_Single<true ,tker>(NpbOk,boundcode,plane,direction,determlimit,dvd,pos,code,idp,velrhop);
		else
			InteractionBoundCorr_Single<false,tker>(NpbOk,boundcode,plane,direction,determlimit,dvd,pos,code,idp,velrhop);
	}
	else if(doublemode==3){
	if(Simulate2D)
		InteractionBoundCorr_Double<true, tker>(NpbOk,boundcode,plane,direction,determlimit,dvd,pos,code,idp,velrhop);
	else
		InteractionBoundCorr_Double<false, tker>(NpbOk,boundcode,plane,direction,determlimit,dvd,pos,code,idp,velrhop);
	}
	else Run_Exceptioon("Double mode calculation is invalid.");
}
...
/// Realiza interaccion entre ghost inlet/outlet nodes y particulas de fluido. GhostNodes-Fluid
//==============================================================================
void JSphCpu::Interaction_BoundCorr(byte doublemode,typecode boundcode,tplane3f plane,tfloat3 direction,float determlimit,const tdouble3 *pos,const typecode *code,const unsigned *idp,tfloat4 *velrhop)
{
	switch(TKernel){
