...
class JCellDivCpuSingle : public JCellDivCpu
{
	...
	public:
		JCellDivCpuSingle(bool stable, bool floating, byte periactive, TpCellMode cellmode, float scell, tdouble3 mapposmin,
			tdouble3 mapposmax, tuint3 mapcells, unsigned casenbound, unsigned casenfixed, unsigned casenpb, JLog2 *log, std::string dirout);
		...
		ullong GetAllocMemory()const{
			return(JCellDivCpu::GetAllocMemory());
		}
		ullong GetAllocMemoryNp()const{
			return(JCellDivCpu::GetAllocMemoryNp());
		};
		ullong GetAllocMemoryNct()const{
			return(JCellDivCpu::GetAllocMemoryNct());
		};
};
