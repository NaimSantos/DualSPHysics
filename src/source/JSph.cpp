...
void JSph::ConfigDomainFixedValue(std::string key, double v){
	...
	if(keyend=="Xmin")
		CfgDomainFixedMin.x=v;
	else if(keyend=="Ymin")
		CfgDomainFixedMin.y=v;
	else if(keyend=="Zmin")
		CfgDomainFixedMin.z=v;
	else if(keyend=="Xmax")
		CfgDomainFixedMax.x=v;
	else if(keyend=="Ymax")
		CfgDomainFixedMax.y=v;
	else if(keyend=="Zmax")
		CfgDomainFixedMax.z=v;
	else
		Run_Exceptioon("Key for limit is invalid.");
}
...
void JSph::ConfigDomainParticlesValue(std::string key, double v){
	const string keyend=(key.size()>=4? key.substr(key.size()-4, 4): "");
	if(keyend=="Xmin")
		CfgDomainParticlesMin.x=v;
	else if(keyend=="Ymin")
		CfgDomainParticlesMin.y=v;
	else if(keyend=="Zmin")
		CfgDomainParticlesMin.z=v;
	else if(keyend=="Xmax")
		CfgDomainParticlesMax.x=v;
	else if(keyend=="Ymax")
		CfgDomainParticlesMax.y=v;
	else if(keyend=="Zmax")
		CfgDomainParticlesMax.z=v;
	else
		Run_Exceptioon("Key for limit is invalid.");
}
...
void JSph::ConfigDomainParticlesPrcValue(std::string key, double v){
	const string keyend=(key.size()>=4? key.substr(key.size()-4, 4): "");
	if(keyend=="Xmin")
		CfgDomainParticlesPrcMin.x=v;
	else if(keyend=="Ymin")
		CfgDomainParticlesPrcMin.y=v;
	else if(keyend=="Zmin")
		CfgDomainParticlesPrcMin.z=v;
	else if(keyend=="Xmax")
		CfgDomainParticlesPrcMax.x=v;
	else if(keyend=="Ymax")
		CfgDomainParticlesPrcMax.y=v;
	else if(keyend=="Zmax")
		CfgDomainParticlesPrcMax.z=v;
	else
		Run_Exceptioon("Key for limit is invalid.");
}
...
void JSph::ConfigDomainResize(std::string key, const JCaseEParms *eparms){
	...
	if(axis!='x' && axis!='y' && axis!='z')
		Run_Exceptioon("Axis value is invalid.");
	if(key.substr(1, 3)!="min" && key.substr(1, 3)!="max")
		Run_Exceptioon("Key value is invalid.");
	if(key.substr(1, 3)=="min"){
		JCaseEParms::JCaseEParmsPos ps=eparms->GetPosminValue(axis);
		switch(ps.mode){
			case JCaseEParms::DC_Fixed:
				ConfigDomainFixedValue(string("DomainFixed")+key, ps.value);
				break;
			case JCaseEParms::DC_DefValue:
				ConfigDomainParticlesValue(string("DomainParticles")+key, ps.value);
				break;
			case JCaseEParms::DC_DefPrc:
				ConfigDomainParticlesPrcValue(string("DomainParticlesPrc")+key, ps.value/100);
				break;
		}
	}
	else{
...
void JSph::AllocMemoryFloating(unsigned ftcount, bool imposedvel, bool addedforce){
	...
	if(FtLinearVel){ //<vs_fttvel_ini>
		for(unsigned c=0;c<FtCount;c++)
			delete FtLinearVel[c];
		delete[] FtLinearVel;
		FtLinearVel=NULL;
	}
	if(FtAngularVel){
		for(unsigned c=0;c<FtCount;c++)
			delete FtAngularVel[c];
		delete[] FtAngularVel; FtAngularVel=NULL;
	}
	if(FtLinearForce){
		for(unsigned c=0;c<FtCount;c++)
			delete FtLinearForce[c];
		delete[] FtLinearForce;
		FtLinearForce=NULL;
	}
	if(FtAngularForce){
		for(unsigned c=0;c<FtCount;c++)
			delete FtAngularForce[c];
		delete[] FtAngularForce;
		FtAngularForce=NULL;
	}
	if(ftcount){
...
llong JSph::GetAllocMemoryCpu()const{
	...
	if(FtObjs)
		s+=sizeof(StFloatingData)*FtCount;
	//-Allocated in other objects.
	if(PartsOut)
		s+=PartsOut->GetAllocMemory();
	if(ViscoTime)
		s+=ViscoTime->GetAllocMemory();
	if(FixedDt)
		s+=FixedDt->GetAllocMemory();
	if(AccInput)
		s+=AccInput->GetAllocMemory();
	if(PartsLoaded)
		s+=PartsLoaded->GetAllocMemory();
	return(s);
}
...
void JSph::LoadConfig(const JSphCfgRun *cfg){
	...
	if(!CaseName.length())
		Run_Exceptioon("Name of the case for execution was not indicated.");
	...
	if(cfg->Sv_Csv&&!WithMpi)
		SvData|=byte(SDAT_Csv);
	if(cfg->Sv_Binx)
		SvData|=byte(SDAT_Binx);
	if(cfg->Sv_Info)
		SvData|=byte(SDAT_Info);
	if(cfg->Sv_Vtk)
		SvData|=byte(SDAT_Vtk);
	SvRes=cfg->SvRes;
	...
	if(!JVtkLib::Available())
		Log->PrintWarning("Code for VTK format files is not included in the current compilation,  so no output VTK files will be created.");
	...
	if(AppInfo.GetCreateDirs()){
		...
		if(DirOut!=DirDataOut)
			fun::MkdirPath(DirDataOut);
	}
	...
	//-PIPS configuration.
	if(cfg->PipsMode)
		DsPips=new JDsPips(Cpu, cfg->PipsSteps, (cfg->PipsMode==2), (TStep==STEP_Symplectic? 2: 1), Log);
}
...
void JSph::LoadKernelSelection(const JSphCfgRun *cfg, const JXml *xml){
	...
	switch(eparms.GetValueInt("Kernel", true, 2)){
		...
		case 2:
			...
			break;
		default:
		Run_Exceptioon("Kernel choice is not valid.");
	}
...
void JSph::LoadConfigCtes(const JXml *xml){
	...
	if(ctes.GetEps()!=0)
		Log->PrintWarning("Eps value is not used (this correction was removed).");
}
...
void JSph::LoadConfigParameters(const JXml *xml){
	JCaseEParms eparms;
	eparms.LoadXml(xml, "case.execution.parameters");
	if(eparms.Exists("FtSaveAce"))
		SaveFtAce=(eparms.GetValueInt("FtSaveAce", true, 0)!=0); //-For Debug.
	...
	if(eparms.Exists("SavePosDouble"))
		SvPosDouble=(eparms.GetValueInt("SavePosDouble", true, 0)!=0);
	...
	switch(eparms.GetValueInt("ViscoTreatment", true, 1)){
		...
		default:
			Run_Exceptioon("Viscosity treatment is not valid.");
	}
	...
	switch(eparms.GetValueInt("Boundary", true, 1)){
		...
		default:
		Run_Exceptioon("Boundary Condition method is not valid.");
	}
	if(TBoundary==BC_MDBC){ //<vs_mddbc_ini>
		...
		switch(eparms.GetValueInt("SlipMode", true, 1)){
			...
			default:
				Run_Exceptioon("Slip mode is not valid.");
		}
	...
	if(eparms.Exists("DeltaSPH")){
		if(eparms.Exists("DensityDT"))
			Run_Exceptioon("The parameters \'DeltaSPH\' and \'DensityDT\' cannot be combined. Only \'DensityDT\' should be used.");
		...
		switch(tddt){
			...
			default:
				if(tddt==2 || tddt==3)
					Run_Exceptioon("Density Diffusion Term \'Fourtakas et al 2019\' is not available in this version.");
				else
					Run_Exceptioon("Density Diffusion Term mode is not valid.");
		}
	...
	if(eparms.Exists("Shifting")){
		...
		switch(eparms.GetValueInt("Shifting", true, 0)){
			...
			default:
				Run_ExceptioonFile("Shifting mode in <execution><parameters> is not valid.", FileXml);
		}
		if(shiftmode!=SHIFT_None){
			shiftcoef=eparms.GetValueFloat("ShiftCoef", true, -2);
			if(shiftcoef==0)
				shiftmode=SHIFT_None;
			else
				shifttfs=eparms.GetValueFloat("ShiftTFS", true, 0);
		}
	...
	if(fun::StrUpper(filefixeddt)=="NONE")
		filefixeddt="";
	if(valuefixeddt && !filefixeddt.empty())
		Run_Exceptioon("The parameters \'DtFixed\' and \'DtFixedFile\' cannot be used at the same time.");
	if(!filefixeddt.empty()){
		FixedDt=new JDsFixedDt();
		FixedDt->LoadFile(DirCase+filefixeddt);
	}
	else if(valuefixeddt)
		FixedDt=new JDsFixedDt(valuefixeddt);

	if(eparms.Exists("RhopOutMin"))
		RhopOutMin=eparms.GetValueFloat("RhopOutMin");
	if(eparms.Exists("RhopOutMax"))
		RhopOutMax=eparms.GetValueFloat("RhopOutMax");
	...
	if(Simulate2D && PeriY)
		Run_Exceptioon("Cannot use periodic conditions in Y with 2D simulations");
	}
	...
	string key;
	if(eparms.Exists(key="DomainFixed")){
	...
	if(eparms.Exists(key="DomainFixedXmin")){
		ConfigDomainFixedValue(key, eparms.GetValueDouble(key));
		resizeold=true;
	}
	if(eparms.Exists(key="DomainFixedYmin")){
		ConfigDomainFixedValue(key, eparms.GetValueDouble(key));
		resizeold=true;
	}
	if(eparms.Exists(key="DomainFixedZmin")){
		ConfigDomainFixedValue(key, eparms.GetValueDouble(key));
		resizeold=true;
	}
	if(eparms.Exists(key="DomainFixedXmax")){
		ConfigDomainFixedValue(key, eparms.GetValueDouble(key));
		resizeold=true;
	}
	if(eparms.Exists(key="DomainFixedYmax")){
		ConfigDomainFixedValue(key, eparms.GetValueDouble(key));
		resizeold=true;
	}
	if(eparms.Exists(key="DomainFixedZmax")){
		ConfigDomainFixedValue(key, eparms.GetValueDouble(key));
		resizeold=true;
	}
	...
	if(resizeold)
		Log->PrintWarning("The options IncZ and DomainFixedXXXX are deprecated.");
...
void JSph::LoadConfigCommands(const JSphCfgRun *cfg){
	...
	switch(cfg->TBoundary){
		...
		default:
			Run_Exceptioon("Boundary method is not valid.");
		}
		if(TBoundary==BC_MDBC)switch(cfg->SlipMode){
			...
			case 3:  SlipMode=SLIP_FreeSlip;  break;
			default:
				Run_Exceptioon("Slip mode for mDBC is not valid.");
		}
		UseNormals=(TBoundary==BC_MDBC);
	}
	if(TBoundary==BC_MDBC && cfg->MdbcThreshold>=0)
		MdbcThreshold=cfg->MdbcThreshold;
	...
	if(cfg->TStep)
		TStep=cfg->TStep;
	if(cfg->VerletSteps>=0)
		VerletSteps=cfg->VerletSteps;
	if(cfg->TVisco){
		TVisco=cfg->TVisco;
		Visco=cfg->Visco;
	}
	if(cfg->ViscoBoundFactor>=0)
		ViscoBoundFactor=cfg->ViscoBoundFactor;
	...
	if(cfg->TDensity>=0){
		switch(cfg->TDensity){
			...
			case 3:  TDensity=DDT_DDT2Full; break;  //<vs_dtt2>
			default:
				if(cfg->TDensity==2 || cfg->TDensity==3)
					Run_Exceptioon("Density Diffusion Term \'Fourtakas et al 2019\' is not available in this version.");
			else
				Run_Exceptioon("Density Diffusion Term mode is not valid.");
		}
	...
	if(TDensity==DDT_None)
		DDTValue=0;
	else if(cfg->DDTValue>=0)
		DDTValue=cfg->DDTValue;
	...
	if(Gravity.z==0){
		if(TDensity==DDT_DDT2)
			Log->PrintWarning("The option DDT:2 is equal to DDT:1 when gravity.z is zero.");
		if(TDensity==DDT_DDT2Full)
			Log->PrintWarning("The option DDT:3 is equal to DDT:1 (but applied to the entire fluid) when gravity.z is zero.");
	}
	...
	TpShifting shiftmode=SHIFT_None;
		switch(cfg->Shifting){
			...
			case 3:  shiftmode=SHIFT_Full;     break;
			default:
				Run_Exceptioon("Shifting mode is not valid.");
		}
		if(!Shifting)
			Shifting=new JSphShifting(Simulate2D, Dp, KernelH, Log);
	...
	if(NstepsBreak)
		Log->PrintfWarning("The execution will be cancelled after %d simulation steps.", NstepsBreak);
	...
	if(cfg->DomainMode==2)
		ConfigDomainFixed(cfg->DomainFixedMin, cfg->DomainFixedMax);
...
void JSph::LoadConfigVars(const JXml *xml){
	if(!JNumexLib::Available())
		Log->PrintWarning("Code for JNumex libary is not included in the current compilation,  so user-defined expresions in XML file are not evaluated.");
	else{
		...
		for(unsigned c=0;c<uvars.CountVars();c++){
			...
			if(v.isnum)
				NuxLib->CreateVar(v.name, true, false, v.valuenum);
			else
				NuxLib->CreateVar(v.name, true, false, v.valuestr);
		}
...
void JSph::LoadCaseConfig(const JSphCfgRun *cfg){
	...
	if(UseChrono){
		if(xml.GetNodeSimple("case.execution.special.chrono", true)){
			if(!JChronoObjects::Available())
				Run_Exceptioon("DSPHChronoLib to use Chrono is not included in the current compilation.");
			ChronoObjects=new JChronoObjects(Log, DirCase, CaseName, &xml, "case.execution.special.chrono", Dp, parts.GetMkBoundFirst());
		}
		else
			Run_ExceptioonFile("Chrono configuration in XML file is missing.", FileXml);
	}
	else if(xml.GetNodeSimple("case.execution.special.chrono", true))
		Log->PrintfWarning("The use of Chrono is disabled (RigidAlgorithm is not 3) although XML file includes the Chrono configuration.");
	//<vs_chroono_end>
	...
	if(xml.GetNodeSimple("case.execution.special.wavepaddles", true)){
		if(!JWaveGen::Available())
			Run_Exceptioon("Code for Wave-Paddles is not included in the current compilation.");
		...
		if(DsMotion)
			for(unsigned ref=0;ref<DsMotion->GetNumObjects();ref++){
	...
	if(xml.GetNodeSimple("case.execution.special.mlayerpistons", true)){
		if(!JMLPistons::Available())
			Run_Exceptioon("Code for Multi-Layer Pistons is not included in the current compilation.");
		bool useomp=false, usegpu=false;
		MLPistons=new JMLPistons(!Cpu, Log, DirCase);
		MLPistons->LoadXml(&xml, "case.execution.special.mlayerpistons");  
		if(DsMotion)
			for(unsigned c=0;c<DsMotion->GetNumObjects();c++){
	...
	if(xml.GetNodeSimple("case.execution.special.relaxationzones", true)){
		if(!JRelaxZones::Available())
			Run_Exceptioon("Code for Relaxation Zones is not included in the current compilation.");
		bool useomp=false, usegpu=false;
		#ifdef OMP_USE_WAVEGEN
			useomp=(omp_get_max_threads()>1);
		#endif
	...
	if(Shifting && xml.GetNodeSimple("case.execution.special.shifting", true))
		Run_ExceptioonFile("Shifting is defined several times (in <special><shifting> and <execution><parameters>).", FileXml);
	if(xml.GetNodeSimple("case.execution.special.shifting", true)){
		Shifting=new JSphShifting(Simulate2D, Dp, KernelH, Log);
		Shifting->LoadXml(&xml, "case.execution.special.shifting");
	}
	if(Shifting && !Shifting->GetShiftMode()){
		delete Shifting;
		Shifting=NULL;
	}
	ShiftingMode=(Shifting? Shifting->GetShiftMode(): SHIFT_None);
	...
	if(FtCount){
		FtConstraints=false;
		if(FtCount>CODE_MKRANGEMAX)
			Run_Exceptioon("The number of floating objects exceeds the maximum.");
		...
		for(unsigned c=0;c<parts.CountBlocks()&&cobj<FtCount;c++){
			...
			if(block.Type==TpPartFloating){
				...
				if(fblock.GetLinearVel()){ //<vs_fttvel_ini>
					...
					if(!FtLinearVel[cobj]->GetFile().empty())
						FtLinearVel[cobj]->LoadFile(DirCase+FtLinearVel[cobj]->GetFile());
				}
				if(fblock.GetAngularVel()){
					FtAngularVel[cobj]=new JLinearValue(*fblock.GetAngularVel());
					if(!FtAngularVel[cobj]->GetFile().empty())
						FtAngularVel[cobj]->LoadFile(DirCase+FtAngularVel[cobj]->GetFile());
				}
				if(fblock.GetLinearForce()){
					FtLinearForce[cobj]=new JLinearValue(*fblock.GetLinearForce());
					if(!FtLinearForce[cobj]->GetFile().empty())
						FtLinearForce[cobj]->LoadFile(DirCase+FtLinearForce[cobj]->GetFile());
				}
				if(fblock.GetAngularForce()){
					FtAngularForce[cobj]=new JLinearValue(*fblock.GetAngularForce());
					if(!FtAngularForce[cobj]->GetFile().empty())
						FtAngularForce[cobj]->LoadFile(DirCase+FtAngularForce[cobj]->GetFile());
				}
				//<vs_fttvel_end>
	...
	if(!WithFloating)
		FtMode=FTMODE_None; //-Disables floatings when there are not floating particles.
	if(UseChrono && PeriActive!=0)
		Log->PrintfWarning("The use of Chrono with periodic limits is only recommended when moving and floating objects do not move beyond those periodic limits."); //<vs_chroono>
		...
		for(unsigned c=0;c<parts.CountBlocks();c++){
			...
			if(IsBound(block.Type)){
				...
				if(ChronoObjects && ChronoObjects->UseDataDVI(mkbound)){
					...
					if(block.Type==TpPartFloating)
						ChronoObjects->ConfigDataBodyFloating(mkbound, data.kfric, data.restitu, data.young, data.poisson);
					if(block.Type==TpPartMoving)
						ChronoObjects->ConfigDataBodyMoving  (mkbound, data.kfric, data.restitu, data.young, data.poisson);
					if(block.Type==TpPartFixed)
						ChronoObjects->ConfigDataBodyFixed   (mkbound, data.kfric, data.restitu, data.young, data.poisson);
				} //<vs_chroono_end>
			}
		}
	}
	...
	if(xml.GetNodeSimple("case.execution.special.inout", true)){
		...
		if(ReuseIds)
			Run_Exceptioon("Inlet/Outlet with ReuseIds is not a valid option for now...");
	}

	//-Configuration of boundary extrapolated correction.
	if(xml.GetNodeSimple("case.execution.special.boundextrap", false))
		Run_Exceptioon("The XML section 'boundextrap' is obsolete.");
	...
	if(xml.GetNodeSimple("case.execution.special.moorings", true)){
		if(WithFloating){
			if(!AVAILABLE_MOORDYN)
				Run_Exceptioon("Code for moorings and MoorDyn+ coupling is not included in the current compilation.");
			Moorings=new JDsMooredFloatings(Log, DirCase, CaseName, Gravity);
			Moorings->LoadXml(&xml, "case.execution.special.moorings");
		}
		else
			Log->PrintWarning("The use of Moorings was disabled because there are no floating objects...");
	}
	...
	if(Moorings || xml.GetNodeSimple("case.execution.special.forcepoints", true)){
		...
		//FtForces->LoadXml(&xml, "case.execution.special.forcepoints");
			...
			Log->PrintWarning("The use of impossed force to floatings was disabled because there are no floating objects...");
	} //<vs_moordyyn_end>

	//-Checks invalid options for symmetry. //<vs_syymmetry_ini>
	if(Symmetry){
		if(Simulate2D)
			Run_Exceptioon("Symmetry is not allowed with 2-D simulations.");
		if(PeriY)
			Run_Exceptioon("Symmetry is not allowed with periodic conditions in axis Y.");
		if(WithFloating)
			Run_Exceptioon("Symmetry is not allowed with floating bodies.");
		if(UseChrono)
			Run_Exceptioon("Symmetry is not allowed with Chrono objects.");
		if(BoundCorr)
			Run_Exceptioon("Symmetry is not allowed with BoundCor.");
		if(TVisco!=VISCO_Artificial)
			Run_Exceptioon("Symmetry is only allowed with Artificial viscosity.");
	} //<vs_syymmetry_end>
...
StDemData JSph::LoadDemData(bool checkdata, const JCasePartBlock* block)const{
	...
	if(checkdata){
		...
		if(!block->ExistsSubValue("Kfric", "value"))
			Run_Exceptioon(objdesc+" - Value of Kfric is invalid.");
		if(!block->ExistsSubValue("Restitution_Coefficient", "value"))
			Run_Exceptioon(objdesc+" - Value of Restitution_Coefficient is invalid.");
		if(!block->ExistsSubValue("Young_Modulus", "value"))
			Run_Exceptioon(objdesc+" - Value of Young_Modulus is invalid.");
		if(!block->ExistsSubValue("PoissonRatio", "value"))
			Run_Exceptioon(objdesc+" - Value of PoissonRatio is invalid.");
	}
	...
	if(block->ExistsValue("Kfric_User"))
		data.kfric=block->GetValueFloat("Kfric_User");
	if(block->ExistsValue("Restitution_Coefficient_User"))
		data.restitu=block->GetValueFloat("Restitution_Coefficient_User");
...
//==============================================================================
void JSph::LoadBoundNormals(unsigned np, unsigned npb, const unsigned *idp, const typecode *code, tfloat3 *boundnormal){
	...
	if(fun::FileExists(filenordata)){
		...
		if(pnorsize){
			if(pnorsize<npb)
				Run_ExceptioonFile("The number of final normals does not match fixed and moving particles.", filenordata);
	...
	Log->Print("**File with normal data not found.");
}

//==============================================================================
/// Config normals to point from boundary particle to ghost node (full distance).
//==============================================================================
void JSph::ConfigBoundNormals(unsigned np, unsigned npb, const tdouble3 *pos, const unsigned *idp, tfloat3 *boundnormal)
{
...
void JSph::RestartCheckData(){
	if(PartBegin){
		...
		if(nmk!=parthead.MkBlockCount())
			Run_ExceptioonFile("Number of Mk blocks is invalid.", filehead);
...
bool JSph::CalcMotion(double stepdt){
	...
	if(ChronoObjects && ChronoObjects->GetWithMotion() && active){ //<vs_chroono_ini> 
		...
		for(unsigned ref=0;ref<nref;ref++){
			const StMotionData& m=DsMotion->GetMotionData(ref);
			if(m.type!=MOTT_None)
				ChronoObjects->SetMovingData(m.mkbound, m.type==MOTT_Linear, m.linmov, m.matmov, stepdt);
		}
	} //<vs_chroono_end>
	return(active);
}
...
void JSph::CalcMotionWaveGen(double stepdt){
	...
	if(WaveGen){
		...
		for(unsigned c=0;c<WaveGen->GetCount();c++){
			if(motsim)
				DsMotion->SetMotionData(WaveGen->GetMotion(svdata, c, TimeStep, stepdt));
			else
				DsMotion->SetMotionDataAce(WaveGen->GetMotionAce(svdata, c, TimeStep, stepdt));
		}
	}
}
...
void JSph::PrintSizeNp(unsigned np, llong size, unsigned allocs)const{
	...
	if(Cpu)
		Log->Printf("**Requested CPU memory for %u particles: %.1f MB.", np, s);
	else
		Log->Printf("**Requested GPU memory for %u particles: %.1f MB (%u times).", np, s, allocs);
}
...
//==============================================================================
void JSph::AddParticlesOut(unsigned nout, const unsigned *idp, const tdouble3 *pos, const tfloat3 *vel, const float *rhop, const typecode *code)
{
...
//==============================================================================
void JSph::AbortBoundOut(JLog2 *log, unsigned nout, const unsigned *idp, const tdouble3 *pos, const tfloat3 *vel, const float *rhop, const typecode *code)
{
...
//==============================================================================
void JSph::AddBasicArrays(JDataArrays &arrays, unsigned np, const tdouble3 *pos , const unsigned *idp, const tfloat3 *vel, const float *rhop)const
{
...
//==============================================================================
void JSph::SaveData(unsigned npok, const JDataArrays& arrays, unsigned ndom, const tdouble3 *vdom, const StInfoPartPlus *infoplus)
{
...
//==============================================================================
void JSph::SaveVtkNormals(std::string filename, int numfile, unsigned np, unsigned npb, const tdouble3 *pos, const unsigned *idp, const tfloat3 *boundnormal)const
{
  if(JVtkLib::Available()){
...
void JSph::SaveRes(float tsim, float ttot, const std::string &headplus, const std::string &detplus){
	...
	if(pf){
		...
		if(pf.fail())
			Run_ExceptioonFile("Failed writing to file.", fname);
		pf.close();
	}
	else
		Run_ExceptioonFile("File could not be opened.", fname);
}
...
void JSph::DgSaveCsvParticlesCpu(std::string filename, int numfile, unsigned pini, unsigned pfin, std::string head, const tfloat3 *pos, const unsigned *idp, const tfloat3 *vel, const float *rhop, const float *ar, const tfloat3 *ace, const tfloat3 *vcorr)
{
	...
	if(pf.fail())
		Run_ExceptioonFile("Failed writing to file.", filename);
	pf.close();
	}
	else
