...
void JDsMotion::ConfigObjects(const JCaseParts *parts){
	...
	if(ObjCount>CODE_MKRANGEMAX)
		Run_Exceptioon("The number of mobile objects exceeds the maximum.");
	//-Prepares memory.
	...
	const JCasePartBlock &block=parts->GetBlock(c);
		if(block.Type==TpPartMoving){
			if(cmot>=ObjCount)
				Run_Exceptioon("The number of mobile objects exceeds the expected maximum.");
	...
	if(cmot!=ObjCount)
		Run_Exceptioon("The number of mobile objects is invalid.");
}

//==============================================================================
/// Initialisation of configuration for moving objects.
//==============================================================================
void JDsMotion::Init(const JCaseParts *parts, JXml *jxml, const std::string &path, const std::string &dirdata)
{
	Reset();
	//-Configures moving objects.
	ConfigObjects(parts);
...
bool JDsMotion::ProcesTime(TpMotionMode mode, double timestep, double dt){
	...
	if(mode==MOMT_Simple)
		ActiveMotion=Mot->ProcesTimeSimple(timestep+TimeMod, dt);
	if(mode==MOMT_Ace2dt)
		ActiveMotion=Mot->ProcesTimeAce(timestep+TimeMod, dt);
	//-Load motion data in ObjMotion[].
	if(ActiveMotion)
		for(unsigned ref=0;ref<ObjCount;ref++){
			...
			if(active){
				...
				if(Simulate2D && typelinear==MOTT_Linear)
					m.linmov.y=m.linvel.y=m.linace.y=0;
			}
...
void JDsMotion::SetMotionData(const StMotionData& d){
	if(d.ref>=GetNumObjects())
		Run_Exceptioon("Moving object does not exist.");
