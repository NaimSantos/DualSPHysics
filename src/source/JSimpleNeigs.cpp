...
unsigned JSimpleNeigs::GetAllocMemory()const{
	unsigned s=0;
	if(PosInCell)
		s+=sizeof(unsigned)*Np;
	if(BeginCell)
		s+=sizeof(unsigned)*(Nct+1);
	if(SelectPos)
		s+=sizeof(unsigned)*SizeSelect;
	return(s);
}
...
void JSimpleNeigs::DefineMapCells(){
	...
	if(ullong(Nct)!=nct0)
		Run_Exceptioon(fun::PrintStr("Number of cells (%d x %d x %d) is invalid.", Ncx, Ncy, Ncz));
}
...
void JSimpleNeigs::CreateMapCells(){
	if(Np==0)
		Run_Exceptioon("Nummber of positions is zero.");
	if(Scell<=0)
		Run_Exceptioon("Size of cells is invalid.");
	DefineMapCells();
	//-Allocate memory.
	...
	BeginCell[0]=0;
	for(int c=0;c<Nct;c++)
		BeginCell[c+1]=BeginCell[c]+npcell[c];
	//-Computes PosInCell[].
	memset(npcell, 0, sizeof(unsigned)*Nct);
	for(unsigned p=0;p<Np;p++){
...
unsigned JSimpleNeigs::NearbyPositions(const tdouble3 &ps, unsigned pignore, double dist){
	...
	for(int cz=celmin.z;cz<=celmax.z;cz++)
		for(int cy=celmin.y;cy<=celmax.y;cy++){
			...
			for(unsigned cp=pini;cp<pfin;cp++){
				...
				if(ds.x*ds.x+ds.y*ds.y+ds.z*ds.z<=dist2 && p!=pignore)
					SelectAdd(p);
			}
	}
	return(CountSelect);
