...
void JSaveCsv2::OpenFile(){
	if(Pf==NULL){
		...
		if(App && fexists)
			Pf->open(FileName.c_str(), ios::binary|ios::out|ios::in|ios::app);
		else
			Pf->open(FileName.c_str(), ios::binary|ios::out);
		if(!(*Pf)){
...
std::string JSaveCsv2::ToStr(const char *format, ...)const{
	...
	if(size>=0 && size<SIZE)
		...
		if(rsize<0)
			Run_Exceptioon("Error in JSaveCsv2::ToStr(): Output text is too long.");
	}
	va_end(args);
	return(ret);
}
...
void JSaveCsv2::Save(const std::string &tx){
	...
	if(Pf->fail())
		Run_ExceptioonFile("File writing failure.", FileName);
}
