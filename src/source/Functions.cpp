...
int GetWeekNumber(int day, int month, int year){
	...
	int v=-1;
	if(tx.size()==2)
		v=int(unsigned(tx[0]-'0')*10+unsigned(tx[1]-'0'));
	return(v);
}
...
std::string PrintStr(const char *format, ...){
	...
	if(size>=0 && size<SIZE)
		...
		if(rsize<0)
			Run_ExceptioonFun("Output text is too long.");
	}
	va_end(args);
	return(ret);
}
...
std::string UintStrFill(unsigned v, unsigned vmax, const char fillchar){
	...
	while(unsigned(value.length())<len)
		value=fill+value;
	return(value);
}
...
tint3 StrToInt3(std::string v){
	tint3 res=TInt3(0);
	if(!v.empty())
		res.x=atoi(fun::StrSplit(", ", v).c_str());
	if(!v.empty())
		res.y=atoi(fun::StrSplit(", ", v).c_str());
	if(!v.empty())
		res.z=atoi(fun::StrSplit(", ", v).c_str());
	return(res);
}
...
std::string StrUpper(const std::string &cad){
	std::string ret;
	for(unsigned c=0;c<cad.length();c++)
		ret=ret+char(toupper(cad[c]));
	return(ret);
}
...
std::string StrTrim(const std::string &cad){
	std::string ret;
	int lsp=0, rsp=0;
	for(int c=0;c<int(cad.length())&&cad[c]==' ';c++)
		lsp++;
	for(int c=int(cad.length())-1;c<int(cad.length())&&cad[c]==' ';c--)
		rsp++;
...
int StrFileToVector(const std::string &file, std::vector<std::string> &lines){
	...
	if(pf){
		...
		if(!pf.eof()&&pf.fail())
			error=1; //-Error: Failure reading data from file.
		pf.close();
	}
...
int StrVectorToFile(const std::string &file, const std::vector<std::string> &lines){
	...
	if(!pf)error=3; //-Error: File could not be opened.
		...
		for(unsigned r=0;r<rows && !error;r++){
			...
			if(pf.fail())
				error=4; //-Error: File writing failure.
		}
		pf.close();
	}
	return(error);
...
//==============================================================================
unsigned VectorFind(const std::string &key, const std::vector<std::string> &vec, unsigned first)
{
	unsigned c=first;
...
unsigned VectorFindMask(const std::string &keymask, const std::vector<std::string> &vec, unsigned first){
	...
	for(unsigned c=first;c<size && ret==UINT_MAX;c++){
		...
		if(!usemask && v==keymask)
			ret=c;
		if(usemask && FileMask(v, keymask))
			ret=c;
...
int CompareVersions(std::string v1, std::string v2){
	int ret=0;
	if(!v1.empty() && (v1[0]=='v' || v1[0]=='V'))
		v1=v1.substr(1);
	if(!v2.empty() && (v2[0]=='v' || v2[0]=='V'))
		v2=v2.substr(1);
...
int FileType(const std::string &name){
	...
	else if(FileSize(name)>=0)
		ret=2; //-Solution for 4GB files unsupported by stat().
	return(ret);
}
...
ullong FileModifTime(const std::string &name){
	...
	struct stat stfileinfo;
	int intstat=stat(name.c_str(), &stfileinfo);
	if(intstat==0 && stfileinfo.st_mode&S_IFREG)
		ret=ullong(stfileinfo.st_mtime);
	return(ret);
}
...
int MkdirPath(std::string path){
	int ret=0;
	//:printf("\n--> path: [%s]\n", path.c_str());
	for(unsigned c=0;c<unsigned(path.size());c++)
		if(path[c]=='\\')path[c]='/';
	//:printf("----> path: [%s]\n", path.c_str());
	if(!path.empty() && !DirExists(path)){
		...
		while(!aux.empty()){
			...
			//:printf("------> dir: [%s]\n", dir.c_str());
			if(!dir.empty() && dir!="."){
				if(path0.empty() && !linuxroot)
					path0=dir; else path0=path0+"/"+dir;
				//:printf("------> path0: [%s]\n", path0.c_str());
			...
			if(!ret.empty()){
				if(ret!="."){
					if(ret==".."){
						...
						dir=dir+(!dir.empty() && dir[dir.size()-1]!='/'? "/": "")+ret;
				}
			}
			else if(dir.empty())
				dir="/";
...
std::string GetDirWithoutSlash(const std::string &ruta){
	char last=ruta[ruta.length()-1];
	if(last=='\\'||last=='/')
		return(ruta.substr(0, ruta.length()-1));
	return(ruta);
}
...
void GetFileNameSplit(const std::string &file, std::string &dir, std::string &fname, std::string &fext){
	...
	if(!fext.empty())
		fname=fname.substr(0, fname.size()-fext.size()-1);
}
...
std::string AddExtension(const std::string &file, const std::string &ext){
	...
	if(file2.empty()||file2[file2.length()-1]!='.')
		file2+='.';
	file2+=ext;
	return(file2);
}
...
std::string ShortFileName(const std::string &file, unsigned maxlen, bool withpoints){
	...
	if(file.length()<=maxlen)
		...
		if(pos1<0||(pos2>=0&&pos2<pos1))
			pos1=pos2;
		if(pos1>=0)
			file2=file2.substr(pos1);
		if(withpoints){
			if(file2.length()+3>maxlen)
				file2=ShortFileName(file2, maxlen-3, false);
...
byte* ResizeAlloc(byte *data, unsigned ndata, unsigned newsize){
	...
	if(ndata)
		memcpy(data2, data, sizeof(byte)*ndata);
	delete[] data;
	return(data2);
}
//==============================================================================
word* ResizeAlloc(word *data, unsigned ndata, unsigned newsize){
	...
	ndata=std::min(ndata, newsize);
	if(ndata)
		memcpy(data2, data, sizeof(word)*ndata);
	delete[] data;
	return(data2);
}
//==============================================================================
unsigned* ResizeAlloc(unsigned *data, unsigned ndata, unsigned newsize){
	...
	ndata=std::min(ndata, newsize);
	if(ndata)
		memcpy(data2, data, sizeof(unsigned)*ndata);
	delete[] data;
	return(data2);
}
//==============================================================================
tuint2* ResizeAlloc(tuint2 *data, unsigned ndata, unsigned newsize){
	...
	ndata=std::min(ndata, newsize);
	if(ndata)
		memcpy(data2, data, sizeof(tuint2)*ndata);
	delete[] data;
	return(data2);
}
//==============================================================================
tuint3* ResizeAlloc(tuint3 *data, unsigned ndata, unsigned newsize){
	...
	ndata=std::min(ndata, newsize);
	if(ndata)
		memcpy(data2, data, sizeof(tuint3)*ndata);
	delete[] data;
	return(data2);
}
//==============================================================================
tuint4* ResizeAlloc(tuint4 *data, unsigned ndata, unsigned newsize){
	...
	ndata=std::min(ndata, newsize);
	if(ndata)
		memcpy(data2, data, sizeof(tuint4)*ndata);
	delete[] data;
	return(data2);
}
//==============================================================================
...
tint2* ResizeAlloc(tint2 *data, unsigned ndata, unsigned newsize){
	...
	ndata=std::min(ndata, newsize);
	if(ndata)
		memcpy(data2, data, sizeof(tint2)*ndata);
	delete[] data;
	return(data2);
}
//==============================================================================
tint3* ResizeAlloc(tint3 *data, unsigned ndata, unsigned newsize){
	...
	ndata=std::min(ndata, newsize);
	if(ndata)
		memcpy(data2, data, sizeof(tint3)*ndata);
	delete[] data;
	return(data2);
}
//==============================================================================
float* ResizeAlloc(float *data, unsigned ndata, unsigned newsize){
	...
	ndata=std::min(ndata, newsize);
	if(ndata)
		memcpy(data2, data, sizeof(float)*ndata);
	delete[] data;
	return(data2);
}
//==============================================================================
tfloat2* ResizeAlloc(tfloat2 *data, unsigned ndata, unsigned newsize){
	...
	ndata=std::min(ndata, newsize);
	if(ndata)
		memcpy(data2, data, sizeof(tfloat2)*ndata);
	delete[] data;
	return(data2);
}
//==============================================================================
tfloat3* ResizeAlloc(tfloat3 *data, unsigned ndata, unsigned newsize){
	...
	ndata=std::min(ndata, newsize);
	if(ndata)
		memcpy(data2, data, sizeof(tfloat3)*ndata);
	delete[] data;
	return(data2);
}
//==============================================================================
tfloat4* ResizeAlloc(tfloat4 *data, unsigned ndata, unsigned newsize){
	...
	ndata=std::min(ndata, newsize);
	if(ndata)
		memcpy(data2, data, sizeof(tfloat4)*ndata);
	delete[] data;
	return(data2);
}
//==============================================================================
double* ResizeAlloc(double *data, unsigned ndata, unsigned newsize){
	...
	ndata=std::min(ndata, newsize);
	if(ndata)
		memcpy(data2, data, sizeof(double)*ndata);
	delete[] data;
	return(data2);
}
//==============================================================================
tdouble2* ResizeAlloc(tdouble2 *data, unsigned ndata, unsigned newsize){
	...
	ndata=std::min(ndata, newsize);
	if(ndata)
		memcpy(data2, data, sizeof(tdouble2)*ndata);
	delete[] data;
	return(data2);
}
//==============================================================================
tdouble3* ResizeAlloc(tdouble3 *data, unsigned ndata, unsigned newsize){
	...
	ndata=std::min(ndata, newsize);
	if(ndata)
		memcpy(data2, data, sizeof(tdouble3)*ndata);
	delete[] data;
	return(data2);
}
//==============================================================================
tdouble4* ResizeAlloc(tdouble4 *data, unsigned ndata, unsigned newsize){
	...
	ndata=std::min(ndata, newsize);
	if(ndata)
		memcpy(data2, data, sizeof(tdouble4)*ndata);
	delete[] data;
	return(data2);
}
