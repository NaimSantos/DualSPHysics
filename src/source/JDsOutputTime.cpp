...
void JDsOutputTime::Config(double timeoutdef){
	Reset();
	if(timeoutdef<=0)
		Run_Exceptioon("Value timeout by default is invalid. It is not greater than zero.");
...
void JDsOutputTime::ReadXml(const JXml *sxml, TiXmlElement* ele){
	...
	while(elet){
		...
		if(AddTimeOut(t, v))
			sxml->ErrReadElement(elet, "tout", false);
...
double JDsOutputTime::GetNextTime(double t){
	if(LastTimeInput>=0 && t==LastTimeInput)
