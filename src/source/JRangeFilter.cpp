...
void JRangeFilter::ResizeRanges(unsigned size){
	unsigned* rg=NULL;
	if(size)
		rg=new unsigned[size*2];
	if(size&&Size)
		memcpy(rg,Ranges,sizeof(unsigned)*2*Size);
	delete[] Ranges;
	Ranges=rg;
	Size=size;
...
bool JRangeFilter::CheckNewValue(unsigned v)const{
	bool ret=false;
	for(unsigned c=0;c<Count&&!ret;c++)
		ret=(Ranges[c<<1]<=v&&v<=Ranges[(c<<1)+1]);
	return(ret);
}
...
void JRangeFilter::AddRangeStep(unsigned v,unsigned v2,unsigned step){ 
	if(step==1)
		...
		for(unsigned c=v;v<=c && c<=v2;c=(c+step>c? c+step: v2+1))
			AddValue(c);
}
...
void JRangeFilter::SortRanges(){
	for(int c=0;c<int(Count)-1;c++){
		...
		for(int c2=c+1;c2<int(Count);c2++)
			if(r>Ranges[c2<<1]){
				Ranges[c<<1]=Ranges[c2<<1];
				Ranges[c2<<1]=r;
				r=Ranges[(c<<1)+1];
				Ranges[(c<<1)+1]=Ranges[(c2<<1)+1];
				Ranges[(c2<<1)+1]=r;
				r=Ranges[c<<1];
		}
	}
...
void JRangeFilter::Config(std::string filter){
	...
	while(!filter.empty()){
		...
		if(!tx.empty()){
			...
			if(!tx1.empty()){
				if(tx2.empty())
					AddValue(atoi(tx1.c_str()));
				else if(tx3.empty())
					AddRange(atoi(tx1.c_str()),atoi(tx2.c_str()));
				else
					AddRangeStep(atoi(tx1.c_str()),atoi(tx2.c_str()),atoi(tx3.c_str()));
			}
		}
	}
	SortRanges();
