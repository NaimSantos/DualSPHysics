...
void JArraysGpuSize::SetArrayCount(unsigned count){
	if(count>MAXPOINTERS)
		Run_Exceptioon("Number of requested arrays exceeds the maximum.");
	if(count<CountUsed)
		Run_Exceptioon("Unable to free arrays in use.");
	if(ArraySize){
		if(Count<count){//-Genera nuevos arrays. //-Generates new arrays
			for(unsigned c=Count;c<count;c++)
				cudaMalloc((void**)(Pointers+c), ElementSize*ArraySize);
...
void* JArraysGpuSize::Reserve(){
	if(CountUsed==Count||!ArraySize)
		Run_Exceptioon(fun::PrintStr("There are no arrays available with %u bytes.",ElementSize));
	CountUsed++;
...
void JArraysGpuSize::Free(void *pointer){
	if(pointer){
		...
		if(pos==MAXPOINTERS)
			Run_Exceptioon("The pointer indicated was not reserved.");
		if(pos+1<CountUsed){
			void *aux=Pointers[CountUsed-1];
			Pointers[CountUsed-1]=Pointers[pos];
			Pointers[pos]=aux;
		}
		CountUsed--;
	}
