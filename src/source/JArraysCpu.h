...
class JArraysCpu : protected JObject
{
	public:
		typedef enum{
			SIZE_1B=1,SIZE_2B=2,SIZE_4B=4,SIZE_8B=8,SIZE_12B=12,SIZE_16B=16,SIZE_24B=24,SIZE_32B=32
	...
	protected:
		...
		JArraysCpuSize* GetArrays(TpArraySize tsize)const{
			return(tsize==SIZE_32B? Arrays32b: (tsize==SIZE_24B? Arrays24b: (tsize==SIZE_16B? Arrays16b: (tsize==SIZE_12B? Arrays12b: (tsize==SIZE_8B? Arrays8b: (tsize==SIZE_4B? Arrays4b: (tsize==SIZE_2B? Arrays2b: Arrays1b)))))));
		}
	...
	public:
		...
		byte* ReserveByte(){
			return((byte*)Arrays1b->Reserve());
		}
		word* ReserveWord(){
			return((word*)Arrays2b->Reserve());
		}
		unsigned* ReserveUint(){
			return((unsigned*)Arrays4b->Reserve());
		}
		int* ReserveInt(){
			return((int*)Arrays4b->Reserve());
		}
		float* ReserveFloat(){
			return((float*)Arrays4b->Reserve());
		}
		tfloat3* ReserveFloat3(){
			return((tfloat3*)Arrays12b->Reserve());
		}
		tfloat4* ReserveFloat4(){
			return((tfloat4*)Arrays16b->Reserve());
		}
		double* ReserveDouble(){
			return((double*)Arrays8b->Reserve());
		}
		tdouble2* ReserveDouble2(){
			return((tdouble2*)Arrays16b->Reserve());
		}
		tdouble3* ReserveDouble3(){
			return((tdouble3*)Arrays24b->Reserve());
		}
		tsymatrix3f* ReserveSymatrix3f(){
			return((tsymatrix3f*)Arrays24b->Reserve());
		}
		#ifdef CODE_SIZE4
			typecode* ReserveTypeCode(){
				return(ReserveUint());
			}
		#else
			typecode* ReserveTypeCode(){
