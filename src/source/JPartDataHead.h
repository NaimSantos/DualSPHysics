...
class JPartDataHeadMkBlock
{
	public:
		...
		JPartDataHeadMkBlock(TpParticles type,unsigned mk,unsigned mktype,unsigned begin,unsigned count):Bound(IsBound(type)),Type(type),Mk(mk),MkType(mktype),Begin(begin),Count(count){ }
};
...
class JPartDataHead : protected JObject
{
	...
	public:
		...
		~JPartDataHead();
		void Reset();
		void ConfigBasic(std::string runcode,std::string appname,std::string casename,tdouble3 caseposmin,tdouble3 caseposmax,bool data2d,double data2dposy,unsigned npieces,unsigned firstpart);
