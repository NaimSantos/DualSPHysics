...
void JCaseVtkOut::Reset(){
	for(unsigned c=0;c<Files.size();c++)
		delete Files[c];
	Files.clear();
}
...
std::string JCaseVtkOut::GetFile(unsigned idx)const{
	if(idx>=Count())
		Run_Exceptioon("Number of requested file name is invalid.");
...
std::string JCaseVtkOut::GetFileListMk(unsigned idx)const{
	if(idx>=Count())
		Run_Exceptioon("Number of requested file info is invalid.");
...
unsigned JCaseVtkOut::GetFiles(std::string key, std::vector<std::string> &list)const{
	...
	for(unsigned ipos=0;ipos<Count();ipos++){
		if(int(Files[ipos]->File.find(key))>=0)
			list.push_back(Files[ipos]->File);
	}
...
std::string JCaseVtkOut::GetListMkType(bool bound, const std::string &mks)const{
	...
	for(unsigned c=0;c<nv;c++){
		...
		if(bound  && v>=MkBoundFirst)
			txmks=txmks+", "+fun::UintStr(v-MkBoundFirst);//-For mkbound.
		if(!bound && v>=MkFluidFirst && v<MkBoundFirst)
			txmks=txmks+", "+fun::UintStr(v-MkFluidFirst);//-For mkfluid.
	}
	rg.Config(txmks);
	return(rg.ToString());
}
...
void JCaseVtkOut::WriteXml(JXml *sxml, TiXmlElement* lis)const{
	...
	for(unsigned c=0;c<Count();c++){
		...
		if(!mklist.empty())
			JXml::AddAttribute(&item, "mkbound", mklist);
		mklist=GetListMkType(false, mks);
		if(!mklist.empty())
			JXml::AddAttribute(&item, "mkfluid", mklist);
